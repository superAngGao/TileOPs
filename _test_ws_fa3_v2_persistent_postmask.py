                ...
                # ===== WG1 (consumer 1, 128 <= tx < 256) =====
                    ...
                    for tile_b, tile_h, tile_m in T.Persistent(
                        ...
                        for n_idx in T.Pipelined(loop_range, num_stages=0):
                            ...
                            # K loop body: ALWAYS clear, no inline mask.
                            # Mask is applied AFTER wait_wgmma, where the
                            # wgmma has already finished writing acc_s_1
                            # so the conditional doesn't conflict and
                            # TileLang's data flow analysis won't insert
                            # extra wait_wgmma<0> calls before each iter.
                            T.clear(acc_s_1)
                            if n_idx == 0:
                                if gi_kc1 % 2 == 0:
                                    T.wgmma_gemm(
                                ...
                                # Post-wgmma mask: only for diagonal block
                                if is_causal:
                ...
                # ===== WG2 (consumer 2, tx >= 256) =====
                    ...
                    for tile_b, tile_h, tile_m in T.Persistent(
                        ...
                        for n_idx in T.Pipelined(loop_range, num_stages=0):
                            ...
                            # Mask applied AFTER wgmma, see WG1.
                            T.clear(acc_s_2)
                            if n_idx == 0:
                                if gi_kc2 % 2 == 0:
                                    T.wgmma_gemm(
