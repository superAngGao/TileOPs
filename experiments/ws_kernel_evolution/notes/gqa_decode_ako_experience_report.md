...
本报告总结本轮将 AKO-style 方法应用到 TileOps GQA decode kernel 调试中的过程、设计取舍和阶段性结论。

本轮探索的目标有两个：

```text
1. 用 AKO-style 闭环调试 GQA decode kernel。
   GQA decode 的组合空间较大，既有 kernel 内部复杂性，也有 dispatch 策略复杂性。

2. 熟悉并本地化 AKO 方法。
   为后续调试 GDN 等 linear attention kernel 建立可复用的实验制度、记录方式和决策口径。
```
...
本报告按三个问题展开：第一，AKO 这套方法本身如何组织 kernel 优化；第二，GQA decode 这个问题是否适合用 AKO；第三，我们如何把 AKO 变体化，用来调试和 tune TileOps GQA decode kernel。
...
1. Workload 定义
   ...
   本轮变体:
       手工固定 GQA decode workload surface：
           Llama4 group=5
           Qwen3.5 group=8
           S=4K 到 128K
           split=16 / 32，以及必要细扫
