#!/usr/bin/env python3
"""Measure PR 871 persistent tail split after wait<0>.
...
Measured WG1 steady-state tail (`n_idx > 0`) after `wait_wgmma(0)`:

  t0: immediately before `barrier_arrive(v_empty)`
  t1: immediately after `barrier_arrive(v_empty)`
  t2: after `T.copy(acc_s_1, acc_s_cast_1)`
                            ...
                            for sub_idx in range(2):
                                ...
                                for n_idx in T.Pipelined(loop_range, num_stages=0):
                                    ...
                                    if n_idx == 0:
                                        ...
                                        T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=2), t2 - t0)
...
if __name__ == "__main__":
    ...
    print(f"Building kernel ({args.variant} tail split)...")
    ...
    if count > 0:
        ...
        print(f"acc_s_cast_copy:    {t[1] / count:.0f} cycles")
        print(f"sum:                {t[2] / count:.0f} cycles")
