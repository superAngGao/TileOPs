#!/usr/bin/env python3
"""Measure PR 871 persistent kernel steady-state region cycles.

This is a milestone-specific clock probe derived from the PR 871 persistent
causal kernel family. It supports both:

  - `base`: original PR 871 persistent ordering
  - `reorder`: KV-head-friendly persistent ordering

It instruments WG1 steady-state iterations
(`n_idx > 0`) with the following timestamps:

  t0: before QK issue
  t1: after PV issue, before wait<1>
  t2: after wait<1>, before softmax
  t3: after softmax, before wait<0>
  t4: after wait<0>
...
This mirrors the table format used by the existing parser in
`run_timeline_probes.py`.
"""
...
def build_clock_kernel(
    ...
    def func(block_m: int, block_n: int):
        ...
        def main(
            ...
            with T.Kernel(
                effective_num_sms, 1, 1, threads=384, prelude=CLOCK_PRELUDE
            ) as (bx, _by, _bz):
                            ...
                            for sub_idx in range(2):
                                ...
                                for n_idx in T.Pipelined(loop_range, num_stages=0):
                                    ...
                                    if n_idx == 0:
                                        ...
                                        T.call_extern("handle", "clk::clock_accum_count", timing.access_ptr("w", offset=4))
                                    gi_kc1 = gi_kc1 + 1
                                rescale_1(acc_o_1, ss_1)
                                T.barrier_wait(v_full, gi_vc1 % 2)
                                if gi_vc1 % 2 == 0:
...
if __name__ == "__main__":
    ...
    timing = torch.zeros(5, device="cuda", dtype=torch.int64)

    print(f"Building kernel ({args.variant})...")
    ...
    if count > 0:
        ...
        for label, cyc in zip(labels, avg):
            print(f"  {label:<18} {cyc:>10.0f} {100 * cyc / total:>7.1f}%")
        print(f"  {'TOTAL':<18} {total:>10.0f} {100.0:>7.1f}%")
