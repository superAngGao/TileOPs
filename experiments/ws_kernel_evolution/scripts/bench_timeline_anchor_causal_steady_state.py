#!/usr/bin/env python3
"""Measure anchor causal persistent kernel steady-state region cycles.

This probe mirrors the PR 871 steady-state table but uses the anchor causal
kernel schedule, including:

- named-barrier WG handoff
- `wait_wgmma_anchor<1>`
- delayed rescale
- `wait_wgmma_anchor<0>`
...
def build_clock_kernel(B: int, S: int, H: int, Hkv: int, D: int):
    if H % Hkv != 0 or D != 128:
        raise ValueError("anchor causal probe expects H % Hkv == 0 and D == 128")
...
if __name__ == "__main__":
    ...
    print("Building kernel (anchor_causal)...")
