#!/usr/bin/env python3
"""Measure anchor causal steady-state core with finer-grained splits.

Measured WG1 steady-state (`n_idx > 0`) sub-regions:

- `qk_issue`: QK `wgmma_gemm` issue only
- `rescale_before_pv`: always 0 for anchor; delayed to tail by design
- `wait_v_full`: wait for V readiness
- `pv_issue`: PV `wgmma_gemm` issue only
- `wait1_fence`: named-barrier handoff + `wait_wgmma_anchor<1>` + acc_s fence
- `post_wait1_preamble`: `k_empty` handoff plus optional last-iter causal mask
- `softmax_core`: softmax body only
- `wait0_fence`: `wait_wgmma_anchor<0>` + acc_o fence
"""
...
def build_clock_kernel(B: int, S: int, H: int, Hkv: int, D: int):
    if H % Hkv != 0 or D != 128:
        raise ValueError("anchor core split expects H % Hkv == 0 and D == 128")
    ...
    @tilelang.jit(
        ...
        compile_flags=["-O3", "-DENABLE_BF16", "-include", str(ROOT / "_anchor_helper.h")],
    )
                        ...
                        for sub_idx in range(2):
                            ...
                            for n_idx in T.Pipelined(loop_range, num_stages=0):
                                ...
                                if n_idx == 0:
                                    ...
                                    t3 = T.call_extern("int64", "clk::read_clock")
                                    if gi_vc1 % 2 == 0:
                                    ...
                                    t4 = T.call_extern("int64", "clk::read_clock")
                                    T.call_extern("handle", "tl::barrier_arrive_named", 2, 256)
                                    T.call_extern("handle", "tl::wait_wgmma_anchor<1>", T.address_of(anchor_sink[0]), gi_kc1)
                                    T.warpgroup_fence_operand(acc_s_1, num_regs=64)
                                    t5 = T.call_extern("int64", "clk::read_clock")
                                    T.barrier_arrive(k_empty)
                                    if n_idx == loop_range - 1:
                                        for i, j in T.Parallel(half_m, block_n):
                                            acc_s_1[i, j] = T.if_then_else(row_base + i >= n_idx * block_n + j, acc_s_1[i, j], -T.infinity(accum_dtype))
                                    t6 = T.call_extern("int64", "clk::read_clock")
                                    softmax_1(acc_s_1, sm_1, smp_1, ss_1, ssum_1, ls_1)
                                    t7 = T.call_extern("int64", "clk::read_clock")
                                    ...
                                    t8 = T.call_extern("int64", "clk::read_clock")
                                    ...
                                    T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=2), t3 - t2)
                                    T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=3), t4 - t3)
                                    T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=4), t5 - t4)
                                    T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=5), t6 - t5)
                                    T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=6), t7 - t6)
                                    T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=7), t8 - t7)
                                    T.call_extern("handle", "clk::clock_accum_count", timing.access_ptr("w", offset=8))
                                gi_kc1 = gi_kc1 + 1
...
if __name__ == "__main__":
    ...
    timing = torch.zeros(9, device="cuda", dtype=torch.int64)

    print("Building kernel (anchor_causal core split)...")
    ...
    if count > 0:
        ...
        total = avg.sum()
        for label, cyc in zip(labels, avg):
            print(f"{label}: {cyc:.0f} cycles")
        print(f"sum_cycles: {total:.0f} cycles")
