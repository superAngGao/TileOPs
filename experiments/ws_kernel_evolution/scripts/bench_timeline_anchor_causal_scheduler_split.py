#!/usr/bin/env python3
"""Measure anchor causal front-end split before QK issue.

This probe isolates the steady-state front-end in WG1 (`n_idx > 0`):

  t_a: before `barrier_wait(k_full, ...)`
  t_b: after `barrier_wait(k_full, ...)`, before scheduler sync
  t_c: after `sync_threads(barrier_id=1, arrive_count=256)`, before QK
...
- `scheduler_sync`
- `sum`
- `clear(acc_s) = 0` because anchor uses `clear_accum=True` on QK issue
  instead of an explicit `T.clear(acc_s)` front-end step
"""
...
def build_clock_kernel(B: int, S: int, H: int, Hkv: int, D: int):
    if H % Hkv != 0 or D != 128:
        raise ValueError("anchor causal split probe expects H % Hkv == 0 and D == 128")
    block_m = 128
    block_n = 128
    ...
    def func(block_m: int, block_n: int):
        ...
        def main(
            ...
            k: T.Tensor((B, S, Hkv, D), "float16"),
            v: T.Tensor((B, S, Hkv, D), "float16"),
            output: T.Tensor((B, S, H, D), "float16"),
                ...
                if tx < 128:
                    ...
                    for tile_b, tile_hkv, pair_idx, tile_g in T.Persistent(
                        ...
                        for sub_idx in range(2):
                            ...
                            for n_idx in T.Pipelined(loop_range, num_stages=0):
                                t_a = T.call_extern("int64", "clk::read_clock")
                                T.barrier_wait(k_full, gi_kc1 % 2)
                                t_b = T.call_extern("int64", "clk::read_clock")
                                T.sync_threads(barrier_id=1, arrive_count=256)
                                t_c = T.call_extern("int64", "clk::read_clock")
                                if n_idx == 0:
                                    if gi_kc1 % 2 == 0:
                                    ...
                                    T.copy(acc_s_1, acc_s_cast_1)
                                    gi_vc1 = gi_vc1 + 1
                                    T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=0), t_b - t_a)
                                    T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=1), t_c - t_b)
...
if __name__ == "__main__":
    ...
    print("Building kernel (anchor_causal scheduler split)...")
    ...
    if count > 0:
        bw = t[0] / count
        sc = t[1] / count
        print(f"barrier_wait(k_full): {bw:.0f} cycles")
        print(f"scheduler_sync:       {sc:.0f} cycles")
        print(f"sum:                  {bw + sc:.0f} cycles")
        print("clear(acc_s) ≈ 0 cycles (handled by clear_accum=True on QK issue)")
    else:
