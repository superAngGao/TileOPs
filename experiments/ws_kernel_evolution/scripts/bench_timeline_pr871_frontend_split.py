#!/usr/bin/env python3
"""Measure PR 871 persistent front-end split before QK issue.
...
Measured front-end segments for WG1 steady-state (`n_idx > 0`):

  t_a: before `barrier_wait(k_full, ...)`
  t_b: after `barrier_wait(k_full, ...)`
  t_c: after `barrier_wait(wg_sched_21, ...)`
  t_d: after `T.clear(acc_s_1)`
                        ...
                        for tile_b, tile_h, pair_idx in T.Persistent(
                            ...
                            for sub_idx in range(2):
                                ...
                                for n_idx in T.Pipelined(loop_range, num_stages=0):
                                    ...
                                    T.clear(acc_s_1)
                                    t_d = T.call_extern("int64", "clk::read_clock")
                                    if n_idx == 0:
                                        if gi_kc1 % 2 == 0:
                                        ...
                                        T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=2), t_d - t_c)
                                        T.call_extern("handle", "clk::clock_accum_count", timing.access_ptr("w", offset=3))
                                    gi_kc1 = gi_kc1 + 1
...
if __name__ == "__main__":
    ...
    timing = torch.zeros(4, device="cuda", dtype=torch.int64)

    print(f"Building kernel ({args.variant} scheduler split)...")
    ...
    if count > 0:
        ...
        print(f"clear(acc_s):        {clr:.0f} cycles")
        print(f"sum:                  {bw + sc + clr:.0f} cycles")
    else:
