#!/usr/bin/env python3
"""Measure anchor causal tail split after wait<0>.

This probe isolates the steady-state tail in WG1 (`n_idx > 0`) after
`wait_wgmma_anchor<0>` has completed:

  t0: immediately before `barrier_arrive(v_empty)`
  t1: immediately after `barrier_arrive(v_empty)`
  t2: after delayed `rescale(acc_o, ss)`
  t3: after `T.copy(acc_s, acc_s_cast)`
...
def build_clock_kernel(B: int, S: int, H: int, Hkv: int, D: int):
    if H % Hkv != 0 or D != 128:
        raise ValueError("anchor causal tail probe expects H % Hkv == 0 and D == 128")
                        ...
                        for sub_idx in range(2):
                            ...
                            for n_idx in T.Pipelined(loop_range, num_stages=0):
                                ...
                                if n_idx == 0:
                                    ...
                                    T.call_extern("handle", "clk::clock_accum", timing.access_ptr("w", offset=3), t3 - t0)
...
if __name__ == "__main__":
    ...
    print("Building kernel (anchor_causal tail split)...")
    ...
    count = t[4]
    if count > 0:
        print(f"v_empty_handoff:    {t[0] / count:.0f} cycles")
        print(f"delayed_rescale:    {t[1] / count:.0f} cycles")
        print(f"acc_s_cast_copy:    {t[2] / count:.0f} cycles")
        print(f"sum:                {t[3] / count:.0f} cycles")
