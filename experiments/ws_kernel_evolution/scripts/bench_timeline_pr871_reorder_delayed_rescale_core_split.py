#!/usr/bin/env python3
"""Measure PR 871 reorder steady-state core with delayed rescale only.

Measured WG1 steady-state (`n_idx > 0`) sub-regions:

- `qk_issue`: QK `wgmma_gemm` issue only
- `rescale_before_pv`: 0 in this experiment because rescale is delayed to tail
- `wait_v_full`: wait for V readiness
- `pv_issue`: PV `wgmma_gemm` issue only
- `wait1_fence`: scheduler handoff + `wait<1>` + `warpgroup_fence_operand(acc_s)`
- `post_wait1_preamble`: `k_empty` handoff plus optional last-iter causal mask
- `softmax_core`: softmax body only
- `wait0_fence`: `wait<0>` + `warpgroup_fence_operand(acc_o)`
"""
...
def build_clock_kernel(
    ...
    if variant != "reorder":
        raise ValueError("this probe only supports reorder with delayed rescale")
...
if __name__ == "__main__":
    ...
    print("Building kernel (reorder_delayed_rescale core split)...")
