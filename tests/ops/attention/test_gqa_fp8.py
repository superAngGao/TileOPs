...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            193, 193, True, -1, -1, 0.0, None, torch.float16,
            id="square-causal-tail",
        ),
        pytest.param(
            191, 191, False, 48, 16, 2.0, 0.125, torch.bfloat16,
            id="square-bidirectional-window-softcap-tail",
        ),
        pytest.param(
            129, 257, True, 96, -1, 2.0, 0.11, torch.float16,
...
def test_gqa_prefill_fp8_native_general_semantic_matrix(
    ...
    q_deq = (q_deq * q_scale[:, None, :, None, None]).reshape(
        batch, seq_len_q, heads, dim
    )
    ...
    center = q_pos + offset
    visible = torch.ones(
        (seq_len_q, seq_len_kv), dtype=torch.bool, device="cuda"
    )
    if is_causal:
