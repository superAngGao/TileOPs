...
def _apply_test_rope(
    ...
    if rope_layout == "neox":
        ...
        rotated = torch.stack(
            (even * selected_cos - odd * selected_sin,
             odd * selected_cos + even * selected_sin),
            dim=-1,
        )
...
def _stand_in(real: type, *, allow_cpu: bool = False) -> type:
    ...
    class StandIn(real):
        ...
        supported_archs = (
            [*real.supported_archs, 0] if allow_cpu else real.supported_archs
        )
...
def _stand_in_dense_prefill_map() -> dict:
    ...
    return {
        key: _stand_in(cls, allow_cpu=True)
        for key, cls in _SHIPPED_DENSE_PREFILL_MAP.items()
    }
...
def _gqa_prefill_ref(
    ...
    *,
    ...
    scores = scores.masked_fill(
        ~visible.view(1, 1, seq_len_q, seq_len_kv), float("-inf")
    )
...
def test_gqa_fwd(batch: int, seq_len: int, heads: int, heads_kv: int, dim: int, causal: bool,
    ...
    op = GroupedQueryAttentionPrefillDenseFwdOp(batch, heads, heads_kv, seq_len, dim, causal, tune=tune)
...
def test_gqa_prefill_dense_fused_rope(
    rope_layout: str, rope_base: float
) -> None:
    ...
    ref = _gqa_prefill_ref(
        q_rot, k_rot, v, heads=heads, heads_kv=heads_kv, is_causal=True
    )
    op = GroupedQueryAttentionPrefillDenseFwdOp(
        batch, heads, heads_kv, seq_len_q, dim, True,
        seq_len_kv=seq_len_kv, pos_encoding_mode="rope",
        rotary_dim=rotary_dim, rope_layout=rope_layout, rope_base=rope_base,
    )
...
def test_gqa_prefill_dense_native_fp8_fused_rope() -> None:
    ...
    k_positions = torch.arange(seq_len_kv, device="cuda", dtype=torch.long).expand(batch, -1)
    q_rot = _apply_test_rope(
        q.to(torch.float16), q_positions, cos, sin, rotary_dim, "interleaved"
    ).to(fp8).to(torch.float16)
    k_rot = _apply_test_rope(
        k.to(torch.float16), k_positions, cos, sin, rotary_dim, "interleaved"
    ).to(fp8).to(torch.float16)
    ref = _gqa_prefill_ref(
        q_rot, k_rot, v.to(torch.float16), heads=heads, heads_kv=heads_kv,
        is_causal=True,
    )
    scale = torch.ones((batch, heads_kv), device="cuda", dtype=torch.float32)
    op = GroupedQueryAttentionPrefillDenseFwdOp(
        batch, heads, heads_kv, seq_len_q, dim, True, seq_len_kv=seq_len_kv,
        dtype=torch.float16, pos_encoding_mode="rope", rotary_dim=rotary_dim,
...
def test_gqa_prefill_dense_validates_constructor_rope_mode() -> None:
    ...
    with pytest.raises(ValueError, match="pos_encoding_mode must be one of"):
        GroupedQueryAttentionPrefillDenseFwdOp(
            **common, pos_encoding_mode="alibi"
        )
    ...
    with pytest.raises(ValueError, match="rope_base must be finite and positive"):
        GroupedQueryAttentionPrefillDenseFwdOp(
            **common, pos_encoding_mode="rope", rope_base=0.0
        )
...
def test_gqa_prefill_dense_fused_rope_cuda_graph_replay() -> None:
    ...
    v = torch.randn_like(k)
    op = GroupedQueryAttentionPrefillDenseFwdOp(
        batch, heads, heads_kv, seq_len, dim, True,
        pos_encoding_mode="rope", rope_layout="interleaved",
    )
    op(q, k, v)
...
@pytest.mark.parametrize(
    (
        "seq_len_q", "seq_len_kv", "is_causal", "window_size_left",
        "window_size_right", "sm_scale", "softcap", "dim", "dtype",
    ),
    [
        pytest.param(97, 193, True, -1, -1, None, None, 64, torch.float16,
                     id="rectangular-causal-tail"),
        pytest.param(129, 257, True, 96, -1, 0.11, 2.0, 64, torch.float16,
                     id="rectangular-causal-left-window-softcap"),
        pytest.param(127, 191, False, 48, 16, 0.125, 50.0, 64, torch.float16,
                     id="bidirectional-window-softcap"),
        pytest.param(96, 160, False, -1, 32, None, None, 64, torch.bfloat16,
                     id="right-window-bf16"),
        pytest.param(97, 193, True, -1, -1, None, None, 128, torch.float16,
                     id="rectangular-causal-dim128-tail"),
        pytest.param(191, 191, True, 96, -1, None, None, 128, torch.float16,
                     id="sliding-dim128-tail"),
    ],
)
@pytest.mark.smoke
...
def test_dense_prefill_path_rejects_unsupported_dtype() -> None:
    ...
    with pytest.raises(ValueError, match="float16 or torch.bfloat16"):
        GroupedQueryAttentionPrefillDenseFwdOp(
            1, 8, 2, 128, 128, True
        )._get_kernel(torch.float32, device=torch.device("cuda"))
    with pytest.raises(ValueError, match="float16 or torch.bfloat16"):
        GroupedQueryAttentionPrefillFwdOp(
            batch=1,
            heads=8,
...
def test_dense_and_packed_prefill_have_distinct_runtime_abis() -> None:
    ...
    assert all(
        issubclass(kernel_cls, PackedPrefillKernel)
        for kernel_cls in _SHIPPED_PREFILL_MAP.values()
    )
