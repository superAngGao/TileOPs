"""FA3-aligned IntraWGOverlap v2 — PERSISTENT + SPLIT PV BUFFER.

Derived from `_test_ws_fa3_v2_persistent_anchor.py`.

Root-cause analysis of the 10% FA3 gap (SASS diff, issue #12):
FA3 uses a **split accumulator** design:
  - acc_pv (R112–R216): PV WGMMA writes here exclusively
  - acc_o  (R13–R108):  running softmax accumulation (rescale target)
  Since the two register sets never overlap, ptxas can schedule:
    wait<1> → softmax → rescale(acc_o) → wait<0> → acc_o += acc_pv

TileOPs (anchor/delay variants) uses a SINGLE acc_o accumulator:
  PV WGMMA *accumulates into* acc_o (ScaleOut::One, clear_accum=False).
  This creates a RAW acc_o conflict between the in-flight PV and the
  rescale FFMA, forcing ptxas to insert wait<0> before rescale.
  Result: only 10–16 insns in the wait<1>→wait<0> window instead of 163.

Fix (this file):
  Introduce `acc_pv_1 / acc_pv_2` fragment (same shape as acc_o).
  PV WGMMA uses clear_accum=True → writes fresh into acc_pv each iter.
  Rescale runs BEFORE wait<0>, reading/writing acc_o only.
  After wait<0>: acc_o += acc_pv  (64 independent FADDs, ~2 cycles).
  This matches FA3's register schedule exactly.

Source ordering per iter (n_idx != 0):
  QK → PV(→acc_pv) → wait<1> → softmax → rescale(acc_o) → wait<0>
  → acc_o[i,j] = acc_pv[i,j]*ss[i] + acc_o[i,j]  (FFMA) → copy

Epilogue:
  PV(→acc_pv) → wait<0> → acc_o += acc_pv  (no extra rescale needed:
  last steady-state iter already rescaled acc_o to m_{N-1}, epilogue
  PV uses acc_s_cast at m_{N-1} too).
...
def build_fa3_v2_persistent(B, S, H, Hkv, D, is_causal,
    ...
    @tilelang.jit(
        ...
        compile_flags=[
            "-O3", "-DENABLE_BF16",
            "-include", "/home/ga/TileOPs/_anchor_helper.h",
        ])
    def func(block_m: int, block_n: int):
        ...
        def main(
            ...
            with T.Kernel(
                ...
                acc_pv_1 = T.alloc_fragment([half_m, D], accum_dtype)
                ...
                acc_pv_2 = T.alloc_fragment([half_m, D], accum_dtype)
                ...
                # Shared-memory sink for wait_wgmma_anchor. Two slots so
                # WG1/WG2 never race. The store is guarded by a predicate
                # that is practically never true (cond == 0xdeadbeef), so
                # no actual writes occur at runtime — but the PTX branch
                # is preserved and anchors the softmax region.
                ...
                # ===== WG1 (consumer 1, 128 <= tx < 256) =====
                    ...
                    for tile_b, tile_hkv, tile_m, tile_g in T.Persistent(
                            ...
                            if n_idx == 0:
                                ...
                                if gi_kc1 % 2 == 0:
                                    ...
                                    T.wgmma_gemm(
                                        ...
                                        policy=T.GemmWarpPolicy.FullRow,
                                        clear_accum=True)
                                # Split-buf: PV writes into acc_pv (fresh
                                # each iter), leaving acc_o free for rescale.
                                ...
                                softmax_1(acc_s_1, sm_1, smp_1,
                                          ss_1, ssum_1, ls_1)
                                # Pre-wait rescale: acc_o has no conflict
                                # with acc_pv (separate registers), so
                                # ptxas defers wait<0> past rescale.
                                rescale_1(acc_o_1, ss_1)
                                # wait<0>: acc_pv (PV result) ready.
                                T.call_extern("handle",
                                              "tl::wait_wgmma_anchor<0>",
                                              T.address_of(anchor_sink[0]),
                                              gi_kc1)
                                T.warpgroup_fence_operand(
                                    acc_pv_1, num_regs=64)
                                T.barrier_arrive(v_empty)
                                # Merge PV result: acc_pv is at m_{n-1} scale,
                                # must rescale by ss before adding to acc_o.
                                # FFMA: acc_o[i,j] = acc_pv[i,j]*ss[i] + acc_o[i,j]
                                # Matches FA3 SASS: FMUL R112+,R10 then FADD.
                                for i, j in T.Parallel(half_m, D):
                                    acc_o_1[i, j] = acc_pv_1[i, j] * ss_1[i] + acc_o_1[i, j]
                                T.copy(acc_s_1, acc_s_cast_1)
                                gi_vc1 = gi_vc1 + 1
                            gi_kc1 = gi_kc1 + 1
                        # Consumer 1 epilogue: PV into acc_pv, merge after wait<0>.
                        # Pre-wait rescale in the last steady-state iter already
                        # scaled acc_o to m_{N-1}; epilogue just adds final PV term.
                        T.barrier_wait(v_full, gi_vc1 % 2)
                        if gi_vc1 % 2 == 0:
                            T.wgmma_gemm(
                                acc_s_cast_1, v_smem_0, acc_pv_1,
                                policy=T.GemmWarpPolicy.FullRow,
                                clear_accum=True)
                        else:
                            T.wgmma_gemm(
                                acc_s_cast_1, v_smem_1, acc_pv_1,
                                policy=T.GemmWarpPolicy.FullRow,
                                clear_accum=True)
                        T.wait_wgmma(0)
                        T.warpgroup_fence_operand(
                        ...
                        T.barrier_arrive(v_empty)
                        gi_vc1 = gi_vc1 + 1
                        # Merge epilogue PV into acc_o, then output.
                        for i, j in T.Parallel(half_m, D):
                ...
                # ===== WG2 (consumer 2, tx >= 256) =====
                    ...
                    for tile_b, tile_hkv, tile_m, tile_g in T.Persistent(
                            ...
                            if n_idx == 0:
                                ...
                                if gi_kc2 % 2 == 0:
                                    ...
                                    T.wgmma_gemm(
                                        ...
                                        policy=T.GemmWarpPolicy.FullRow,
                                        clear_accum=True)
                                # Split-buf: PV into acc_pv_2 (separate regs).
                                ...
                                softmax_2(acc_s_2, sm_2, smp_2,
                                          ss_2, ssum_2, ls_2)
                                # Pre-wait rescale: no conflict with acc_pv_2.
                                ...
                                T.call_extern("handle",
                                              ...
                                              gi_kc2)
                                T.warpgroup_fence_operand(
                                    acc_pv_2, num_regs=64)
                                T.barrier_arrive(v_empty)
                                # Merge PV result: acc_pv_2 is at m_{n-1} scale,
                                # must rescale by ss_2 before adding to acc_o_2.
                                for i, j in T.Parallel(half_m, D):
                                    acc_o_2[i, j] = acc_pv_2[i, j] * ss_2[i] + acc_o_2[i, j]
                                T.copy(acc_s_2, acc_s_cast_2)
                                gi_vc2 = gi_vc2 + 1
                            gi_kc2 = gi_kc2 + 1
                        # Consumer 2 epilogue: PV into acc_pv_2, merge after wait<0>.
                        T.barrier_wait(v_full, gi_vc2 % 2)
                        if gi_vc2 % 2 == 0:
                            T.wgmma_gemm(
                                acc_s_cast_2, v_smem_0, acc_pv_2,
                                policy=T.GemmWarpPolicy.FullRow,
                                clear_accum=True)
                        else:
                            T.wgmma_gemm(
                                acc_s_cast_2, v_smem_1, acc_pv_2,
                                policy=T.GemmWarpPolicy.FullRow,
                                clear_accum=True)
                        T.wait_wgmma(0)
                        T.warpgroup_fence_operand(
                        ...
                        gi_vc2 = gi_vc2 + 1
                        for i, j in T.Parallel(half_m, D):
                            acc_o_2[i, j] = acc_o_2[i, j] + acc_pv_2[i, j]
