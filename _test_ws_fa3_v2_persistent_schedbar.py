"""FA3-aligned IntraWGOverlap v2 — PERSISTENT + DELAYED RESCALE + SCHEDBAR.

Derived from `_test_ws_fa3_v2_persistent_delay_rescale.py`.

Diagnosis (verified by PTX diff): delay_rescale's generated PTX has the
CORRECT FA3-like structure (wait<1> → ~630 PTX lines of softmax → wait<0>),
but ptxas compiles it to SASS with wait<1> and wait<0> only 48 bytes apart
(softmax moved past both waits) because there is NO CONDITIONAL BRANCH in
the softmax region. FA3 has branches (elected-thread-guarded mbarrier
arrives for pipeline_k/v.consumer_release) that act as scheduling fences
for ptxas.

This file tests whether a DUMMY conditional branch between wait_wgmma(1)
and softmax is enough to anchor wait<0> below the softmax. The branch is
guarded by a runtime cumulative counter (`gi_kc1 == 999999999`) which
ptxas cannot constant-fold away, forcing the branch into the final SASS.
                    ...
                    for tile_b, tile_hkv, tile_m, tile_g in T.Persistent(
                        ...
                        for n_idx in T.Pipelined(loop_range, num_stages=0):
                            ...
                            if n_idx == 0:
                                ...
                                softmax_1(acc_s_1, sm_1, smp_1,
                                          ss_1, ssum_1, ls_1)
                                # schedbar: branch BELOW softmax, ABOVE
                                # wait<0> — anchors softmax above wait<0>
                                # so PV can overlap with softmax execution.
                                if gi_kc1 == 999999999:
                                    T.call_extern(
                                        "handle", "__threadfence_block")
                                T.wait_wgmma(0)
                                T.warpgroup_fence_operand(
                                    acc_o_1, num_regs=64)
