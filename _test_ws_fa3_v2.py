"""FA3-aligned IntraWGOverlap v2: double-buffered K/V, separate pipelines.

Matches FA3's architecture:
  - K pipeline: k_smem[2], k_full barrier (producer→consumer), k_empty barrier (consumer→producer)
  - V pipeline: v_smem[2], v_full barrier, v_empty barrier
  - k_empty/v_empty arrive_count=256 (both consumer WGs arrive)
  - Early K release after wait<1>, early V release after wait<0>
  - No fence_proxy_async (mbarrier + wgmma.fence suffices)
  - TL_DISABLE_THREAD_STORAGE_SYNC for true async WS

Buffer selection via TIR-level `if n_idx % 2 == 0` / `else`.
"""
import time
