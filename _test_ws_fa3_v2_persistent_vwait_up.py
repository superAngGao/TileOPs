"""FA3-aligned IntraWGOverlap v2 — PERSISTENT + DELAYED RESCALE + VWAIT_UP.

Derived from `_test_ws_fa3_v2_persistent_delay_rescale.py`.

Delay_rescale SASS (verified empirically) still produced
  QK → wait<0> → softmax → PV → wait<0>
instead of FA3's
  QK → PV → wait<1> → softmax → wait<0>.

Root cause (re-analysed): ptxas hoisted softmax above PV commit because
softmax only depends on acc_s (QK output), not on PV. To make acc_s
available before softmax runs, ptxas inserted `wait<0>` right after QK
and let softmax run serially before committing PV. The source's explicit
`wait_wgmma<1>` became a no-op (only 1 WGMMA pending at that point) and
was promoted to wait<0> for PV retirement.

FA3 SASS (between last QK HGMMA and first PV HGMMA) contains ONLY the
V mbarrier try_wait — no WGMMA DEPBAR — so ptxas had no slot to insert
a wait<0>+softmax block there. QK and PV commit back-to-back.

Fix (this file): move `T.barrier_wait(v_full)` from between QK and PV
commits to BEFORE QK. After the change, the source code between
QK commit_batch and PV commit_batch is empty (modulo warpgroup_arrive,
which FA3 also has). ptxas should keep QK and PV adjacent and push
softmax below PV commit, yielding the FA3 pattern.

Source ordering (n_idx != 0 branch, this file):
  wait(v_full) → QK → PV → wait<1> → softmax → wait<0> → rescale → copy
  (delay_rescale: QK → wait(v_full) → PV → wait<1> → softmax → wait<0> → rescale → copy)

Epilogue: unchanged (no rescale needed, last softmax+rescale already
scaled acc_o to m_{N-1}).
                    ...
                    for tile_b, tile_hkv, tile_m, tile_g in T.Persistent(
                        ...
                        for n_idx in T.Pipelined(loop_range, num_stages=0):
                            ...
                            if n_idx == 0:
                                ...
                                # vwait_up: wait for V[n-1] BEFORE QK so
                                # the source between QK and PV commits is
                                # empty. Prevents ptxas from hoisting
                                # softmax into that slot.
