...
def gated_deltanet_fwd_roofline(op: Any | None = None, **kwargs: Any) -> tuple[int, int]:
    ...
    # Per recurrent head and token: two state matvecs, one state outer-product
    # update, and the q-k dot product.
    flops = 2 * batch * seq_len * value_heads * (3 * dim_k * dim_v + dim_k)
    ...
    state = batch * value_heads * dim_v * dim_k
