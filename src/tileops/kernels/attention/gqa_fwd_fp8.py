...
def _gqa_fwd_fp8_general_kernel(
    ...
    def build(block_m: int, block_n: int, num_stages: int, threads: int) -> Callable:
        ...
        def main(
            ...
            with T.Kernel(
                T.ceildiv(seq_len_q, block_m), heads, batch, threads=threads
            ) as (bx, by, bz):
                ...
                for i, d in T.Parallel(block_m, dim):
                    ...
                    if q_pos < seq_len_q:
                        if fuse_rope:
                            ...
                            q_shared[i, d] = rotate(
                                q[bz, q_pos, by, d], q[bz, q_pos, by, paired_d],
                                offset + q_pos, d, cos_table, sin_table)
                        else:
                ...
                if is_causal:
                    k_end = T.ceildiv(
                        T.min(seq_len_kv, (bx + 1) * block_m + offset), block_n
                    )
                ...
                if window_size_left >= 0:
                    k_start = T.max(
                        0, bx * block_m + offset - window_size_left
                    ) // block_n
                else:
                    k_start = 0

                for k_offset in T.Pipelined(
                    T.max(k_end - k_start, 0), num_stages=num_stages
                ):
...
def _(
    ...
    *inputs: Tuple[torch.Tensor, ...],
    ...
    return torch.empty(
        (batch, seq_len_q, heads, dim), dtype=dtype, device=inputs[0].device
    )
