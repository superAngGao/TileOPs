...
def _validate_same_device(reference: torch.Tensor, **tensors: torch.Tensor) -> None:
    ...
    for name, tensor in tensors.items():
        if tensor.device != reference.device:
            raise ValueError(
                f"{name} must be on the same device as q ({reference.device}), "
                f"got {tensor.device}"
            )
...
def _validate_rope_config(
    ...
    if rope_layout not in _ROPE_LAYOUTS:
        raise ValueError(
            f"rope_layout must be one of {sorted(_ROPE_LAYOUTS)}, got {rope_layout!r}"
        )
    if fuse_rope:
...
def _resolve_rope_tables(
    ...
    *,
    ...
    if rope_cos.dtype != table_dtype or rope_sin.dtype != table_dtype:
        raise ValueError(
            "rope_cos and rope_sin must have the attention output dtype "
            f"({table_dtype})"
        )
...
def _resolve_group_scales(
    ...
    *,
    ...
    if any(scale is None for scale in scales):
        ...
        if identity is None:
            identity = torch.ones(
                (batch, heads_kv), device=reference.device, dtype=torch.float32
            )
    ...
    def _uniform_cu_seqlens(
        ...
        if cu_seqlens is None:
            cu_seqlens = torch.arange(
                self.batch + 1, device=device, dtype=torch.int32
            ) * seq_len
...
class GroupedQueryAttentionPrefillDenseFwdOp(Op):
    ...
    def __init__(self,
        ...
        if fuse_rope and seq_len > seq_len_kv:
            raise ValueError(
                "fused RoPE uses bottom-right Q positions and requires "
                "seq_len <= seq_len_kv"
            )
        self.batch = batch
        self.heads = heads
        ...
        self.rotary_dim = _validate_rope_config(
            dim, fuse_rope, rotary_dim, rope_layout
        )
    ...
    def _validate_dtypes(
        ...
        if q.dtype not in allowed_inputs:
            raise ValueError(
                "q.dtype must be float16, bfloat16, or float8_e4m3fn, "
                f"got {q.dtype}"
            )
        ...
        if is_fp8:
            ...
            if hasattr(self, "output_dtype") and output_dtype not in (
                ...
                raise ValueError(
                    "dtype must select a float16 or bfloat16 output for FP8 input"
                )
        else:
            output_dtype = getattr(self, "output_dtype", None)
    ...
    def _eager_forward(
        ...
        has_scales = tuple(
            scale is not None for scale in (q_scale, k_scale, v_scale)
        )
        ...
        q_scale, k_scale, v_scale = self._scales_or_identity(
            q, q_scale, k_scale, v_scale)
