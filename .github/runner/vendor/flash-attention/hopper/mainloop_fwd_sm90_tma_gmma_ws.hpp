...
struct CollectiveMainloopFwdSm90 {
    ...
    load(Params const& params,
...
#if defined(TILEOPS_FA3_SHAPED_FIRST_V_DATA_FROM_TILELANG) && TILEOPS_FA3_SHAPED_FIRST_V_DATA_FROM_TILELANG
            if (smem_pipe_write.count() == 0) {
                // TileOPs probe path: stage 0 V is already materialized by TileLang.
        ...
        if (should_load_KV) {
            ...
            if constexpr (Transpose_V) {
#if (defined(TILEOPS_FA3_SHAPED_SKIP_FIRST_V_LOAD) && TILEOPS_FA3_SHAPED_SKIP_FIRST_V_LOAD) || \
    (defined(TILEOPS_FA3_SHAPED_SKIP_FIRST_V_LOAD_TRANSFORM) && TILEOPS_FA3_SHAPED_SKIP_FIRST_V_LOAD_TRANSFORM) || \
    (defined(TILEOPS_FA3_SHAPED_FIRST_V_DATA_FROM_TILELANG) && TILEOPS_FA3_SHAPED_FIRST_V_DATA_FROM_TILELANG)
                // TileOPs probe path: stage 0 Vt TMA is supplied by TileLang.
#else
        ...
        for (; n_block >= n_block_min; --n_block) {
            ...
            if constexpr (Transpose_V) {
#if defined(TILEOPS_FA3_SHAPED_SKIP_FIRST_V_LOAD_TRANSFORM) && TILEOPS_FA3_SHAPED_SKIP_FIRST_V_LOAD_TRANSFORM
                if (smem_pipe_write_v.count() != 0) { copy_Vt_to_V(smem_pipe_write_v); }
#else
        ...
        if constexpr (Transpose_V) {
#if defined(TILEOPS_FA3_SHAPED_SKIP_FIRST_V_LOAD_TRANSFORM) && TILEOPS_FA3_SHAPED_SKIP_FIRST_V_LOAD_TRANSFORM
            if (smem_pipe_write.count() != 0) { copy_Vt_to_V(smem_pipe_write); }
#else
