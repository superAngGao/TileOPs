...
def _prefill_auto_cp_local_chunks(num_chunks: int, num_heads: int) -> int:
    env_max_local_chunks = os.environ.get("TILEOPS_GDN_PREFILL_CP_MAX_LOCAL_CHUNKS")
...
def _prefill_cp_split_initial_state_bthd(
    k: torch.Tensor,
    v: torch.Tensor,
    A: torch.Tensor,
    g: torch.Tensor,
    ...
    from tileops.kernels.gated_deltanet.cp_split.hopper import (
    ...
    has_cp_split = num_chunks > max_local_chunks
    force_cp = os.environ.get("TILEOPS_GDN_PREFILL_CP_FORCE", "0") == "1"
    use_cp = has_cp_split and (
        force_cp or num_heads <= 40 or (num_heads <= 64 and num_chunks >= 128)
    )
    if not use_cp:
...
def _gated_deltanet_prefill_wrapped_kernel(
    ...
    if layout == "bthd":
        ...
        use_cp_split = (
            os.environ.get("TILEOPS_GDN_PREFILL_CP_SPLIT", "1") != "0"
            and batch == 1
            and use_blocksolve_prepare
        )
        if use_cp_split:
            from tileops.kernels.gated_deltanet.cp_split import fused_gdr_fwd
            ...
            initial_state, cu_seqlens, cp_seq_map, raw_cu_seqlens = (
                _prefill_cp_split_initial_state_bthd(
                    k=k,
                    v=v,
                    A=A,
                    g=g_cum,
